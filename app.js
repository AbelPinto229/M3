...
import { Task, BugTask, FeatureTask } from './TaskClasses.js';
import { processTask } from './taskutils.js';
...
// Ciclo de status para as tarefas
    ...
    .map(t => {
      ...
      <tr class="group hover:bg-slate-50 transition-colors">
        <td class="py-3 font-medium text-slate-700 ${t.status === 'Concluído' ? 'line-through opacity-40' : ''}">
          ${t.title}<span class="text-[8px] font-bold text-slate-400 block uppercase tracking-tighter">${t.getType()}</span>
        </td>
        <td class="py-3 text-center">
          <button onclick="cycleTaskStatus(${state.tasks.indexOf(t)})" class="text-[9px] font-bold px-2 py-1 rounded-md border ${statusColor}">
...
window.cycleTaskStatus = (i) => {
  ...
  task.moveTo(taskStatusCycle[(currentIndex + 1) % taskStatusCycle.length]);
  addLog(`Tarefa: ${task.title} -> ${task.status}`);
  saveAndRender();
};
...
document.getElementById('taskForm').onsubmit = (e) => {
  ...
  let task;
  switch(type) {
    case "bug": task = new BugTask(title); break;
    case "feature": task = new FeatureTask(title); break;
    default: task = new Task(title); break;
  }
