    ...
    render() {
      ...
      <tr class="group hover:bg-slate-50 transition-colors">
        ...
        <td class="py-3 text-center">
          <button onclick="window.uiController.toggleUserStatus(${u.id})" class="text-[9px] font-bold px-2 py-1 rounded-full border ${u.active ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-slate-100 text-slate-400 border-slate-200'}">${u.active ? 'ATIVO' : 'INATIVO'}</button>
        </td>
        <td class="py-3 text-right">
          <button onclick="window.uiController.deleteUser(${u.id})" class="text-slate-300 hover:text-red-500 transition-colors">
