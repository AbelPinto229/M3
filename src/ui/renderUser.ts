...
export class RenderUser {
    ...
    userList.innerHTML = filteredUsers
      .map(
        (u) => {
          ...
          const allTasks = (window as any).taskService.getTasks?.() || [];
  ...
  toggleUserStatus(id: number): void {
    ...
    (window as any).notificationService.addNotification(`${user.name} agora está ${newStatus}!`, 'success');
    (window as any).logService.addLog(`Utilizador ${user.name} estado: ${newStatus}`);
  ...
  deleteUser(id: number): void {
    ...
    if ((window as any).currentUserRole === 'MANAGER' && user.role === 'ADMIN') {
      (window as any).notificationService.addNotification('Gerentes não podem eliminar utilizadores administradores!', 'warning');
      return;
    }
    
    // Trigger confirmation modal
    (window as any).renderModals.openConfirmModal(`Eliminar ${user.email}?`, () => {
      this.userService.deleteUser(id);
      (window as any).notificationService.addNotification(`${user.name} eliminado!`, 'success');
      (window as any).logService.addLog(`Utilizador ${user.name} eliminado`);
      (window as any).saveAndRender();
    });
  }
  ...
  showUserDetails(id: number): void {
    ...
    (window as any).renderModals.openModal(`Detalhes de ${user.name}`, detailsContent);
  }
