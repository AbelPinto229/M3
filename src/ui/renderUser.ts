...
import { addUser, getAllUsers, removeUser, getNextID, userList } from "../services/userService.js";
...
// STATISTICS
// ---------------------------
function updateStats(list: UserClass[]): void {
    const totalUsers = document.getElementById("total-users") as HTMLSpanElement;
    const activeUsers = document.getElementById("active-users") as HTMLSpanElement;
    const inactiveUsers = document.getElementById("inactive-users") as HTMLSpanElement;
...
// RENDER USERS
// ---------------------------
export function renderUsers(list: UserClass[]): void {
...
export function renderUserForm(): void {
    ...
    form.onsubmit = e => {
        ...
        if (photoInput.files && photoInput.files[0]) photo = URL.createObjectURL(photoInput.files[0]);
