...
export class RenderUser {
    ...
    async loadExtraDataForUsers(users) {
        ...
        try {
            ...
            if (changed) {
                for (const r of results) {
                    ...
                    if (watchCounter)
                        watchCounter.textContent = String(r.watcherCount);
    ...
    async setUserVIPLevel(userId, vipLevel) {
        const user = this.userService.getUserById(userId);
        if (!user)
            return;
        ...
        if (level >= 0 && level <= 4) {
            // Save to DB via API
            await this.userService.updateUser(userId, { vip_level: level });
