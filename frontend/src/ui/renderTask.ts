...
export class RenderTask {
  ...
  public renderTaskRow(t: ExtendedTask): string {
    ...
    const priorityBadgeClass = priorityColors[priorityLevel as keyof typeof priorityColors] || 'bg-slate-100 text-slate-600';
    return `
          ...
          <div class="space-y-4 flex flex-col">
            ...
            <div>
              ...
              <select id="dependencySelect" class="w-full text-xs px-2 py-1 rounded border bg-white" onchange="window.appContext.renderTask.addDependency(${taskId}, this.value); this.value='';">
                ...
                ${this.taskService.getTasks().filter((t: any) => t.id !== taskId).map((t: any) => `<option value="${t.id}">${t.title}</option>`).join('')}
              </select>
            </div>
  ...
  async addTag(): Promise<void> {
    ...
    try {
      ...
      let tagId = existingTags.find((t: any) => t.name.toLowerCase() === tagName.toLowerCase())?.id;
  ...
  async cycleTaskStatus(id: number): Promise<void> {
  ...
  async manualAssign(taskId: number, email: string): Promise<void> {
    const task = this.taskService.getTaskById(taskId);
    if (!task) return;
  ...
  async toggleTaskFavorite(taskId: number): Promise<void> {
    const task = this.taskService.getTaskById(taskId);
    if (!task) return;
  ...
  async rateTask(taskId: number, rating: number): Promise<void> {
    const task = this.taskService.getTaskById(taskId);
    if (!task) return;
  ...
  private renderTaskRatings(taskId: number): void {
    const task = this.taskService.getTaskById(taskId);
    if (!task) return;
