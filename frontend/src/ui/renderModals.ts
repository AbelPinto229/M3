...
export class RenderModals {
  private pendingDeleteAction: (() => void | Promise<void>) | null = null;
  ...
  async saveEditTitle(taskId: number): Promise<void> {
  ...
  async openEditUserModal(userId: number, user: any): Promise<void> {
    ...
    const watchersListHTML = watcherCount > 0 
      ? watchersList.map((w: any) => `<li class="text-xs text-slate-700">• ${w.name || w.watcher_name || 'Utilizador'}</li>`).join('')
  ...
  private async performUserUpdate(userId: number, updateData: any): Promise<void> {
  ...
  async saveCreateUser(): Promise<void> {
    ...
    if (photoInput?.files?.length) {
      ...
      reader.onload = async (event) => {
        const newUser = await window.appContext.userService.addUser(
          emailInput.value, 
          nameInput.value, 
          roleSelect.value, 
  ...
  async saveCreateTask(): Promise<void> {
