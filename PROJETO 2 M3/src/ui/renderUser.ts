    ...
    userList.innerHTML = filteredUsers
      ...
      <tr class="group hover:bg-slate-50 transition-colors cursor-pointer" onclick="window.appContext.renderUser.showUserDetails(${u.id})">
        ...
        <td class="py-3 text-right" onclick="event.stopPropagation()">
          <button ${window.appContext.checkPermission?.('edit_user') ? '' : 'disabled'} onclick="${window.appContext.checkPermission?.('edit_user') ? `window.appContext.renderUser.editUser(${u.id})` : 'return false'}" class="text-slate-300 hover:text-indigo-600 transition-colors ${!window.appContext.checkPermission?.('edit_user') ? 'opacity-50 cursor-not-allowed' : ''} mr-2">
