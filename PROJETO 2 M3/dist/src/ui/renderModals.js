...
export class RenderModals {
    ...
    openEditUserModal(userId, user) {
        ...
        const tasksListHTML = assignedTasks.length > 0
            ? assignedTasks.map((t) => `<li class="text-xs text-slate-700">• ${t.title} <span class="text-[9px] px-1.5 py-0.5 rounded bg-slate-100 text-slate-600">${t.status}</span></li>`).join('')
    ...
    saveEditUser(userId) {
        ...
        const vipLevelSelect = document.getElementById('editUserVIPLevel');
